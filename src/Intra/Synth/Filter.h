...
struct FilterCoeffs
{
	...
	bool operator==(decltype(nullptr)) const {return A1 == 0 && A2 == 0 && B1 == 0 && B2 == 0 && C == 1;}
...
struct Filter: FilterCoeffs
{
	...
	Filter(decltype(nullptr)=nullptr): FilterCoeffs{0, 0, 0, 0, 1} {}
...
struct FilterFactory
{
	...
	FilterFactory(decltype(nullptr)=nullptr): Coeffs({0,0,0,0,1}),
	...
	INTRA_FORCEINLINE bool operator==(decltype(nullptr)) const {return Coeffs == nullptr && Type == FilterType::End;}
	INTRA_FORCEINLINE bool operator!=(decltype(nullptr)) const {return !operator==(nullptr);}
...
struct ResonanceFilter
{
	...
	ResonanceFilter(decltype(nullptr)=nullptr): DeltaPhase(0), QFactor(0), PrevSample(0), S(0) {}

	ResonanceFilter(float dphi, float qfactor):
		DeltaPhase(dphi), QFactor(qfactor), PrevSample(0), S(0) {}

	INTRA_FORCEINLINE bool operator==(decltype(nullptr)) const noexcept {return DeltaPhase == 0;}
...
struct ResonanceFilterFactory
{
	...
	INTRA_FORCEINLINE ResonanceFilterFactory(decltype(nullptr) = nullptr): Frequency(0), QFactor(0) {}
	INTRA_FORCEINLINE ResonanceFilterFactory(float frequency, float qfactor):
		Frequency(frequency), QFactor(qfactor) {}

	INTRA_FORCEINLINE bool operator==(decltype(nullptr)) const noexcept {return Frequency == 0;}
	INTRA_FORCEINLINE bool operator!=(decltype(nullptr)) const noexcept {return !operator==(nullptr);}
	INTRA_FORCEINLINE explicit operator bool() const noexcept {return !operator==(nullptr);}
...
struct DynamicResonanceFilter
{
	...
	DynamicResonanceFilter(decltype(nullptr)=nullptr): DeltaPhase(0), InvQFactor(2), InvQFactorStep(0), PrevSample(0), S(0) {}
...
struct DynamicResonanceFilterFactory
{
	...
	INTRA_FORCEINLINE DynamicResonanceFilterFactory(decltype(nullptr) = nullptr): Frequency(0), InvQFactor(0), InvQFactorSpeed(0) {}
...
struct SoftHighPassFilterFactory
{
	...
	INTRA_FORCEINLINE SoftHighPassFilterFactory(decltype(nullptr)=nullptr): CutoffFrequency(0) {}
	INTRA_FORCEINLINE SoftHighPassFilterFactory(float frequency):
		CutoffFrequency(frequency) {}

	INTRA_FORCEINLINE bool operator==(decltype(nullptr)) const noexcept {return CutoffFrequency == 0;}
	INTRA_FORCEINLINE bool operator!=(decltype(nullptr)) const noexcept {return !operator==(nullptr);}
	INTRA_FORCEINLINE explicit operator bool() const noexcept {return !operator==(nullptr);}
