...
class WaveFormSampler: public WaveTableSampler
{
	...
	Span<const float> prepareInternalData(const void* params, WaveForm wave,
...
public:
	...
	template<typename F, typename = Meta::EnableIf<
		...
		>> INTRA_FORCEINLINE WaveFormSampler(const F& wave,
			...
			float vibratoFrequency, float vibratoValue, float smoothingFactor, const Envelope& envelope = nullptr):
