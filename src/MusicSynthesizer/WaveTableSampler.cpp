...
{
	if(dstRight.Empty()) dstRight.Begin = dstRight.End = nullptr;
	if(dstReverb.Empty()) dstReverb.Begin = dstReverb.End = nullptr;
	const float rightMult = dstRight == nullptr? 0.5f: mRightMultiplier;
	const float leftMult = dstRight == nullptr? 0.5f: mLeftMultiplier;
...
{
	if(mRate == 1 && mFreqOscillator == nullptr)
	{
...
Sampler& CreateSampler(float freq, float volume, unsigned sampleRate,
	SamplerContainer& dst, uint16* oIndex = nullptr)
{
